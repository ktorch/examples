...
emb:seq(`sequential`embed; (`embedseq;v;d;n); (`drop;p))
...
blk:{[d;h;p]
 ...
 q2:seq(`sequential; (`layernorm;d);(`linear;d;d*4); `gelu; (`linear;d*4;d); (`drop;p));
 (`sequential; (`residual;q1); (`residual;q2))}[d;h;p]

blk:enlist[`modulelist`blocks],8#enlist blk

end:{[d;v]
 q:seq(`sequential`end; (`layernorm;`norm;d); (`linear;`decode;d;v;0b)); /output logits for each token
 q,enlist(`transform; seq(`sequential; (`reshape;-1,v));                 /train: reshape to match target
                      seq(`sequential; (`select;1;-1)))}[d;v]            /eval: take last prediction

q:module((`callback;`cb;`fwd;`tensor`tensor); emb; blk; end)
...
/ ------------------------------------------------------------------------------
/ fwd - callback function triggered by higher-level forward call (called by c++)
/ ------------------------------------------------------------------------------
fwd:{[m;x;y]
...
opt(o;0;(q;p 0);`decay,0.0) /no weight decay for linear bias, embed or normalizations
opt(o;1;(q;p 1);`decay,0.1) /set weight decay for remaining parameters
...
epoch:{[m;t;u;n;w;s]
 i:(0N,w)#neg[i]?i:count[t]-1+n;  /w indices of length n+1 from random starting points
...
-2 "\n",g[500]"O God, O God!";  / generate 500 chars from starting phrase
