...
q:(`sequential; (`conv2d; 1;20;5); `relu; `drop; (`maxpool2d;2))
q,:            ((`conv2d;20;50;5); `relu; `drop; (`maxpool2d;2); `flatten)
q,:            ((`linear;800;500); `relu; `drop; (`linear;500;10))
q:module seq q
to(q; c)

m:model(q; loss(`ce;`smoothing,.01); opt(`lamb;q;`decay,.1))
...
r:{(k+1)!.0002|.5*x*1+cos acos[-1]*(k:til y)%y}[.01]50
fmt:.Q.fmt'[4 7 9 6;0 4 6 2]
msg:{-2 raze[("";"  lr:";"  training loss:";"  test accuracy:"),'fmt x],"%";}

fit:{[m;a;r;n;i] 
 if[i=n;-2"\nweight averaging:"];
 lr(m;r@:i&n);                  / reduce learning rate w'each epoch until averaging
 msg i,r,raze`run`testrun@\:m;  / report results of train & test steps
 $[i=n; use[a]copyparms m; i>n; avgparms(m;a)]; i+1} / begin averaging after n epochs

a:dict(); n:count[r]-19  /final 20 epochs for averaging weights
count[r] fit[m;a;r;n]/1;
if[not all copyparms(m;a); '"unable to replace all weights with averages"];

-2 "\ntest accuracy using average weights from epoch ",string[n],"-",string[count r],": ",.Q.fmt[6;2;testrun m],"%";
...
-2 "\nmismatches:"; show y
-2 "\ngrid of mismatches: ",1_string png(` sv p,`out`swa.png;g);
